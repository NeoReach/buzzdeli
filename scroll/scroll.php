	...
	if ( ! class_exists( 'SOD_WooCommerce_Infinite_Scroll' ) ) {
		class SOD_WooCommerce_Infinite_Scroll {
			function __construct(){
				...
				load_plugin_textdomain('sod_inf_scroll', false, FRESH_DELI_SCROLL . '/languages');
			}
			...
			function sod_scripts(){
				wp_register_style( 'sod-infinite-style', FRESH_DELI_SCROLL.'assets/css/product-scroll.css' );
				...
				wp_enqueue_script( 'sod-infinite-script', FRESH_DELI_SCROLL.'assets/js/jquery.infinitescroll.js', array( 'jquery' ) );
				wp_enqueue_script( 'sod-custom-script', FRESH_DELI_SCROLL.'assets/js/custom.js', array( 'jquery' ) );
				$data = array( 'image_path' => FRESH_DELI_SCROLL.'assets/images/ajax-loader.gif','loading_msg' => __('Loading More Products...', 'sod_inf_scroll'),'no_more_products' => __('No More Products to Show','sod_inf_scroll') );
