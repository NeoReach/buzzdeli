	...
	if ( ! class_exists( 'SOD_WooCommerce_Infinite_Scroll' ) ) {
		class SOD_WooCommerce_Infinite_Scroll {
			function __construct(){
				...
				load_plugin_textdomain('sod_inf_scroll', false, dirname( plugin_basename( __FILE__ ) ) . '/languages');
			}
			...
			function sod_scripts(){
				wp_register_style( 'sod-infinite-style', plugin_dir_url( __FILE__ ) . 'assets/css/product-scroll.css' );
				...
				wp_enqueue_script( 'sod-infinite-script', plugin_dir_url( __FILE__ ) . 'assets/js/jquery.infinitescroll.js', array( 'jquery' ) );
				wp_enqueue_script( 'sod-custom-script', plugin_dir_url( __FILE__ ) . 'assets/js/custom.js', array( 'jquery' ) );
				$data = array( 'image_path' => plugin_dir_url( __FILE__ ) . 'assets/images/ajax-loader.gif','loading_msg' => __('Loading More Products...', 'sod_inf_scroll'),'no_more_products' => __('No More Products to Show','sod_inf_scroll') );
